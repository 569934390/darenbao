define([
    'text!modules/sg_stockmanager/update/templates/InboundUpdateView.html',
    'modules/sg_stockmanager/add/views/AddInboundWindow',
    'Portal'
], function (InboundUpdateViewTpl,AddInboundWindow,Portal) {
	var GRID_DOM = "#inbound-update-rule-content-div";
	var options = {
	        height: $(window).height()*0.9,
	        modal: false,
	        draggable: false,
	        content: InboundUpdateViewTpl,
	        autoResizable: true
	};
    var popup;
    var inboundId='';
	...
	return {
        ...
        initTree:function(){
        	...
        	var options = {
        		...
        		$("#treeDemoInboundUpdate").tree(options);

        		$("#tree_select_menu_html_inbound_update").position({ 
        			of: $( "#inbound_update-classify_name" ),
        			my: "left top",
        			at: "left bottom"
        		}).hide();

        		$("#inbound-update-menuBtn").click(function(event) {
        			showMenu();
        			return false;
        		});
        		...
        		function onClick(e, treeNode) {
        			/*var nodes = $("#treeDemoInboundUpdate").tree("getSelectedNodes"),
        				v = "";
        			...
        			var cityObj = $("#inbound_update-classify_name");
        			cityObj.attr("value", treeNode.name);
        			$("#inbound_update-classify").val(treeNode.id)
        			hideMenu();
        		}

        		function showMenu() {
        			$("#tree_select_menu_html_inbound_update").slideDown("fast");
        			//$("body").on("mousedown", onBodyDown);
        		}
        		function hideMenu() {
        			$("#tree_select_menu_html_inbound_update").fadeOut("fast");
        			//$("body").on("mousedown", onBodyDown);
        		}
        		$("body").on("mousedown", function(event){
        			if (!(event.target.id == "inbound-update-menuBtn" || event.target.id == "tree_select_menu_html_inbound_update" || $(event.target).parents("#tree_select_menu_html_inbound_update").length>0)) {
        				hideMenu();
        			}
        		});
        },		
		initClick:function(){
			var me=this;
            $('.inbound-update-manager button').click(function(){
                var btnAction = $(this).attr('action');
                switch(btnAction){
                    case 'search-inbound-update': 
                    	 me.pageData(1);
                         break;
                    ...
                    case 'delete-edt-all':
                    	...
                    	if(records==null||records.length<1){
                    		...
                    		t.result.then(function resolve(){
                    			$.post(Portal.webRoot+'/stock/inbound/delInboundOrderDetail.do',{
		...
		refresh:function(){
        	 $('#inbound_update-classify').val('0')
        	 $('#inbound_update-classify_name').val('')
        	 $('#update_brand').val('0')
             $('input[name=update_param]').val('');
        	 this.initTree();
             this.pageData(1);
        },
        pageData:function (page, rowNum, sortname, sortorder) {
            var matchParam = $('input[name=update_param]').val();
            var classify = $('input[name=inbound_update-classify]').val();
            var brand = $('select[name=update_brand]').val();
