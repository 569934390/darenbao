...
angular.module('ionic-ratings', ['ionic'])
  ...
  .directive('ionicRatings', [function() {
    ...
    .directive('ionicRatingsRead', [function() {
      return {
        restrict: 'AE',
        replace: true,

        template: '<div class="{{iconClass}} ionic_ratings">' +
        '<span class="icon {{offIcon}} ionic_rating_icon_off" ng-style="offColor"  ng-show="ratingNum < 1"></span>' +
        '<span class="icon {{onIcon}} ionic_rating_icon_on" ng-style="onColor"  ng-show="ratingNum > 0"></span>' +
        '<span class="icon {{offIcon}} ionic_rating_icon_off" ng-style="offColor"  ng-show="ratingNum < 2"></span>' +
        '<span class="icon {{onIcon}} ionic_rating_icon_on" ng-style="onColor" ng-show="ratingNum > 1"></span>' +
        '<span class="icon {{offIcon}} ionic_rating_icon_off" ng-style="offColor"  ng-show="ratingNum < 3"></span>' +
        '<span class="icon {{onIcon}} ionic_rating_icon_on" ng-style="onColor" ng-show="ratingNum > 2"></span>' +
        '<span class="icon {{offIcon}} ionic_rating_icon_off" ng-style="offColor" ng-show="ratingNum < 4"></span>' +
        '<span class="icon {{onIcon}} ionic_rating_icon_on" ng-style="onColor"  ng-show="ratingNum > 3"></span>' +
        '<span class="icon {{offIcon}} ionic_rating_icon_off" ng-style="offColor"  ng-show="ratingNum < 5"></span>' +
        '<span class="icon {{onIcon}} ionic_rating_icon_on" ng-style="onColor"  ng-show="ratingNum > 4"></span></div>',
        scope: {
          //ratingsObj: '=ratingsobj',
          ionClass:'@',
          iconOn:'@',
        ...
        link: function(scope, element, attrs) {
          ...
          //Setting the default values, if they are not passed
          scope.iconClass=scope.ionClass || 'text-center';
          scope.onIcon = scope.iconOn || 'ion-ios-star';
          scope.offIcon = scope.iconOff || 'ion-ios-star-outline';
          scope.iconOnColor= scope.iconOnColor || 'rgb(200, 200, 100)';
          scope.iconOffColor=  scope.iconOffColor || 'rgb(200, 100, 100)';
          scope.ratingNum=  scope.ratingNum || 1;
          scope.onColor = {
            color: scope.iconOnColor
          };
