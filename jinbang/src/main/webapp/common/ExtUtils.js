 ...
 * @author pan.xiaobo
 */
Ext.define('club.common.ExtUtils', {
    ...
    /**
     *  悬浮层提示窗口
     * ExtUtils.tip('测试','测试{0}','test');
     * @param {} title 标题
     * @param {} format 可格式化的字符串
     */
    tip: function(title, format){
        if(!this.msgCt){
            this.msgCt = Ext.DomHelper.insertFirst(document.body, {style:' position:absolute;left:50%; top:10px;width:400px; margin-left:-200px;z-index:20000;'}, true);
        }
        var s = Ext.String.format.apply(String, Array.prototype.slice.call(arguments, 1));
        title=title||StrConstants.HINT;
        var m = Ext.DomHelper.append(this.msgCt, '<div style="border-radius: 8px; -moz-border-radius: 8px;background: #F6F6F6; border: 2px solid #ccc;margin-top: 2px; padding: 10px 15px; color: #555;"><h3 style="margin: 0 0 8px; font-weight: bold; font-size: 15px;">' + title + '</h3><p style="margin: 0;">' + s + '</p></div>', true);
        m.hide();
        m.show();
//        m.slideIn('t').ghost("t", { delay: 100, remove: true});
        ...
        m.fadeOut({duration:5,delay: 2000,remove:true});
    },
    /**
     * ajax异步请求数据
     *
        ...
        Ext.Ajax.request(Ext.apply({
            ...
            success : function(response) {
                ...
                try {
                    ...
                    if (jsonData.errCode) {
                        var msg = jsonData.msg || jsonData.exception || jsonData.desc || jsonData.errCode;
     ...
     * 检查view是否有选中记录
     * @param {} view 要检查的组件
     * @param {} multiRecord 是否允许选择多条记录
     * @param {} showAlarm 没有选中时是否弹出警告提示
     * @return {Boolean}
     */
    ...
    setParams:function(param,data){
		var operationMap={};
		Ext.apply(param,data);
		for(var key in param){
			operationMap[key]={paramName:key,operation:param.operation||WEBConstants.OPERATION.EqualTo,paramValue:[]};
			if(Ext.isString(data[key])){
				data[key] = Ext.String.trim(data[key]);
				param[key] = data[key];
			}
			if(Ext.isEmpty(data[key]) || data[key]==''){
				delete param[key];
			}else{
				operationMap[key]['paramValue'].push(data[key]);
			}
		}
		var arr=[];
		for (var key in operationMap){
			arr.push(operationMap[key]);
		}
		var conditionStr=Ext.isEmpty(arr)?null:Ext.encode(arr);
		param.queryConditions=conditionStr;
	}
