...
function hideDock(e){
	...
	constans.$collectSet.css({top:constans.height-128,left:constans.width/2-64,width:'128px',height:'128px'}).stop().animate({top:constans.height-40,left:constans.width/2-20,width:'40px',height:'40px'}).animate({top:constans.height-25}).show();
	setTimeout(function(){
...
function closeChart(){
	if(!Portal.chartPanelbg||!Portal.chartPanel)return;
	if($.isIE8m()){
		Portal.chartPanelbg.hide();
		Portal.chartPanel.hide();
	}
	else{
		Portal.chartPanelbg.fadeOut();
		Portal.chartPanel.fadeOut();
	}
}
function openUrl(id,url,fn,scope){
	...
	if(id==='charts'){
		if(!Portal.chartPanel){
			Portal.chartPanelbg = $('<img class="chart-panel-bg" /src="'+webRoot+'portal/images/chart_bg.png" />').hide();
			Portal.chartPanel = $('<div class="chart-panel"><div class="chart-box"></div></div>').hide();
//			Portal.chartPanel.css({width:constans.width,height:constans.height,marginTop:0,marginLeft:0});
			Portal.chartPanelbg.appendTo(document.body);
			Portal.chartPanel.appendTo(document.body);
			Portal.chartPanelbg.click(function(e){
		...
		if($.isIE8m()){
			Portal.chartPanelbg.toggle();
			Portal.chartPanel.toggle();
		}
		else{
			Portal.chartPanelbg.fadeToggle();
			Portal.chartPanel.fadeToggle();
		}
		return;
	}
	closeChart();
	...
	if(constans.mainPanel.find('#'+id).length==0){
		...
		var $iframe = $('<iframe class="main_frame"  id="'+id+'" frameborder="no"   border="no"></iframe>');
		constans.mainPanel.append($iframe);
		$iframe.attr('src',webRoot+taskMenus[index].url);
		$iframe.load(function(){
			$(this).css({width: '100%',height: '100%'});
			constans.pageLoading.hide();
			this.contentWindow.Portal = Portal;
...
$(function(){
	constans.$collectSet = $('<img class="collect" src="'+webRoot+'portal/images/collect.png" />').appendTo(document.body);
	constans.$collectSet.bind('mouseout',function(){
		if(!constans.dockIsHide) return;
		$(this).stop().css({left:constans.width/2-20,width:'40px',height:'40px',top:constans.height-25});
	}).click(function(e){
		showDock(e);
	});
	var rights = [{title:'新窗口打开',handler:function(index){
		window.open(webRoot+taskMenus[index].url);
	}},{title:'关闭当前页',handler:function(index){
		$('iframe').each(function(i,item){
			if($(item).attr('id')==taskMenus[index].id){
		...
		$('iframe').each(function(i,item){
			if($(item).attr('id')==taskMenus[index].id){
				$(item).attr('src',webRoot+taskMenus[index].url);
				return false;
			}
		});
...
//	});
	...
	openUrl(6,'debug/debug.jsp',function(){});
