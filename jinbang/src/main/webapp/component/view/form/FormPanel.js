 ...
 */
Ext.define('component.view.form.FormPanel', {
        ...
        for(var i=0;i<config.items.length;i++){
            if(config.items[i]=='->'&i>0){
                ...
                if(!config.items[i].width){
                    config.items[i].width = WEBConstants.FIELD_DEFAULT_WIDTH;
                }
                if(config.items[i].allowBlank==false){
                    config.items[i].afterSubTpl = WEBConstants.REQUIRED;
                    config.items[i].width = config.items[i].width+WEBConstants.FIELD_REQUIRED_WIDTH;
                }
    ...
    resize:function(f){
        ...
        if(form.height){
            if (me.absHeight) {
                form.height = form.height+me.absHeight;
            };
