 ...
 * @author 	Damen
 * @date 	2013-11-18
 * @define 	component.public.TreeCommonMethod
 * 
 * @title   方法(methods)
...
Ext.define('component.public.TreeCommonMethod', {
...
//						child.fireEvent('checkchange', child, checked);
			...
			me.getRootNode().cascadeBy(function(node){
				...
				if(node.data.id==pathArr[pathArr.length-1]){
					if(node.childNodes.length>0){
						...
						params.parentPath = params.parentPath.replace(pId,"");
	...
	selectNodeByBizId : function(bizId,fieldName,callback,scop){
		...
		me.getRootNode().cascadeBy(function(node){
			var attrs = Ext.JSON.decode(node.raw.attributes);
