Ext.define('component.business.PingPortWindow', {
	...
	constructor : function(config) {
		...
		me.items = [me.createForm(config),
			me.portListViewer = Ext.create('component.business.PortListViewer',{
				tbar:[{
					text:'过滤有效IP',
					iconCls:'toolbar-alarmFilterPolicy',
					handler:Ext.bind(me.fillerIps,me)
				},'-',{
					text:'显示全部',
					iconCls:'toolbar-sync',
					handler:function(){
						me.portListViewer.getStore().clearFilter();
					}
				},'-',{
					text:'接口任务',
					iconCls:'toolbar-tasks',
					handler:Ext.bind(me.showTasks,me)
				},'->',{
					text:'执行',
					...
					handler:function(){
						...
						me.portTaskPanel.setVisible(false);
					}
				}],
				region : 'center'
			}),
			...
			me.portTaskPanel = Ext.create('component.business.PortTaskView',{
				tbar:[{
						text:'返回',
						iconCls:'toolbar-back',
						handler:function(){
		...
		if(records.length==0){
			...
			for(var i=0,length=store.getCount();i<length;i++){
				...
				Ext.apply(params,{
					...
					nodeId:me.config.getNodeId(),
					neConfigName:store.getAt(i).get('NODE_NAME'),
					portStatus:store.getAt(i).get('PORT_STATE'),
					portIndex:store.getAt(i).get('PORT_INDEX')
				});
				me.doCmd(params);
			}
		}
			...
			for(var i=0,length=records.length;i<length;i++){
				...
				Ext.apply(params,{
					...
					nodeId:me.config.getNodeId(),
					neConfigName:records[i].get('NODE_NAME'),
					portStatus:records[i].get('PORT_STATE'),
					portIndex:records[i].get('PORT_INDEX')
				});
				me.doCmd(params);
			}
		}
...
//				inputWidth : 50,
	...
	showTasks:function(){
		...
		me.portListViewer.setVisible(false);
		me.commandPanel.setVisible(false);
		me.portTaskPanel.setVisible(true);
		var store = me.portListViewer.getStore();
		var nodeNameStr = '';
		for(var i=0,length=store.getCount();i<length;i++){
			nodeNameStr+=store.getAt(i).get('NODE_NAME');
			if(i<length-1)
				nodeNameStr+=',';
		}
		me.portTaskPanel.getStore().proxy.extraParams={nodeNameStr:nodeNameStr};
		me.portTaskPanel.getStore().load();
	},
	layout : 'border'
});