Ext.define('component.business.VlanResourceWindow', {
	...
	createCmd : function() {
		...
		var cmd = {
			...
			items : [{
				...
				queryMode:'remote',
				readOnly:true,
				sqlKey:sqlmapPrefix+"ComboCommonMapper.vlanResourceScriptCombo"
			},{
				xtype : 'button',
				...
				for (var i = 0; i < l; i++) {
					...
					if (result.uuid == record.get('nodeId')) {
						if (result.message==true) {
							...
							record.set('result', 'Vlan采集脚本执行完毕!');
						}else {
				...
				(function(record) {
					setTimeout(function() {
						if (record.get('success') == false) {
							...
							record.set('success', true);
							record.set('result', '网元Vlan采集脚本执行超时！');
						}
					}, 180000);
