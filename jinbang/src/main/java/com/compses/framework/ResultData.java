package com.compses.framework;



import com.compses.dto.Page;
import com.compses.util.TortoiseClassUtils;
...
/**
 * 结果数据类
 * Created by gaozhiqiang on 14-12-25.
 */
...
//    private String flag;
    ...
    public ResultData(Class clazz, Object object) {
        ...
        if (object instanceof Page) {
            ...
            this.putEntities(clazz, page.getResult());
            this.setTotal(((Long) page.getTotalCount()).intValue(), page.getLimit());
        }
        //不分页集合处理
    ...
    public <E> ResultData putEntity(Class clz, E e) {
        String key = INFLECTOR.lowerCamelCase(
                INFLECTOR.singularize(TortoiseClassUtils.getEntityNameWithoutDto(clz))
        );
    ...
    public <E> ResultData putEntity(E e) {
        String key = INFLECTOR.lowerCamelCase(
                INFLECTOR.singularize(TortoiseClassUtils.getEntityNameWithoutDto(e.getClass()))
        );
        if (!entityKeys.contains(key)) {
            entityKeys.add(key);
        }
    ...
    private String getPluralKey(Class entityClass) {
        return INFLECTOR.lowerCamelCase(
                INFLECTOR.pluralize(TortoiseClassUtils.getEntityNameWithoutDto(entityClass))
        );
    }
